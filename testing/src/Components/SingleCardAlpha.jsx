...
import PdfCard from "./PdfCard"; // Ensure you have PdfCard component

const SingleCardAlpha = ({ id, apiUrl }) => {
  ...
  const [responseType, setResponseType] = useState(""); // State to store the response type

  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const response = await axios.get(newApiUrl, { responseType: 'arraybuffer' }); // Update responseType
  ...
  const numberOfItems = responseType === 'json' ? data.length : 0;
  const fieldNames = responseType === 'json' && data.length > 0 ? Object.keys(data[0]) : [];
  ...
  return (
    <>
      ...
      <Dialog open={expanded} onClose={handleExpandClose} maxWidth="xl" fullWidth={true}>
        <DialogTitle>
          {responseType === 'pdf' ? 'PDF Viewer' : 'Table Data'}
...
export default SingleCardAlpha;
